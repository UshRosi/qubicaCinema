...
/// <summary>
/// Puts a film, a room and a screening on the programme through Catalog's real endpoints, so every test that
/// needs one does it the same way a client would — never by writing rows straight into <see cref="CatalogApp"/>'s
/// database.
/// </summary>
internal static class CatalogFlows
{
    ...
    /// <summary>Adds a film to the catalogue. The genre and age rating do not matter to the tests that use this.</summary>
    ...
    internal static async Task<HttpResponseMessage> ScheduleScreeningAsync(
        this HttpClient admin,
        Guid movieId,
        Guid auditoriumId,
        ...
        await admin.PostAsJsonAsync(
            "/api/v1/screenings",
            new { movieId, auditoriumId, startsAt, priceAmount, priceCurrency },
            cancellationToken);
